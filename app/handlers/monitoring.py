...
from ..utils.format import human_bytes, human_duration, render_table
...
async def show_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    rows = [
        ...
        (
            "Load",
            ", ".join(f"{val:.2f}" for val in metrics.load_avg),
        ),
        (
            "RAM",
            f"{human_bytes(metrics.mem_total - metrics.mem_available)} / {human_bytes(metrics.mem_total)}",
        ),
        (
            "Disk /",
            f"{human_bytes(metrics.disk_root_total - metrics.disk_root_free)} / {human_bytes(metrics.disk_root_total)}",
        ),
        ("Uptime", human_duration(metrics.uptime_seconds)),
    ]
    if (
        metrics.disk_hdd_total is not None
        and metrics.disk_hdd_free is not None
    ):
        rows.append(
            (
                "Disk HDD",
                f"{human_bytes(metrics.disk_hdd_total - metrics.disk_hdd_free)} / {human_bytes(metrics.disk_hdd_total)}",
            )
        )
